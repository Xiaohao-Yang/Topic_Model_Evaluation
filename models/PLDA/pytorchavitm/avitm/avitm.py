...
class AVITM(object):
    ...
    def fit(self, data_dict, embedding_model, args, save_dir=None):
        """
        ...
        for epoch in range(self.num_epochs):
            ...
            if (epoch + 1) % args.eval_step == 0:
                parameter_setting = 'PLDA_%s_K%s_RS%s_epochs:%s_LR%s' \
                                    % (args.dataset, args.n_topic, args.seed, epoch+1, args.lr)
                evaluation(self.model, parameter_setting, data_dict, args, embedding_model)
                self.model.train()
